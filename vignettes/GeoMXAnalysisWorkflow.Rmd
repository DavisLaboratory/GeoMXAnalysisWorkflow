...
In this instance, using `limma::removeBatchEffect` approach has effectively 
separated the samples based on the disease status on PC1. Furthermore, the 
samples are also reasonable well clusters and separated based on tissue 
regions (particular for tubules) across PC2 and PC3
...
For the downstream analyses like differential expression analyses, standR does not provide specific functions. 
