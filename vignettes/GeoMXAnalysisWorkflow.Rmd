...
The `readGeoMX` function has other parameters such as `hasNegProbe` and `NegProbeName` that are opt to deal with negative probes in the data. Then breifly talking about negprobes in both WTA and CTA.
By default, the `readGeoMx` function will remove the negative probe, the one with name "NegProbe-WTX", in the count table and put it in the metadata of the object. User can turn this off by specifying `hasNegProbe = FALSE` in the function, just make sure no duplicate gene names in the "TargetName" column.
...
In our analysis, it is of interest to see which genes are differential expressed in different tissue regions within different status of the samples, a design matrix is therefore set up with both the disease status (DKD and normal) and tissue regions (glomerulus_healthy, glomerulus_abnormal, tubule_PanCK and tubule_neg) information. We also add the W matrices resulted from the `RUV4` to the model matrix as covariates to use batch corrected data.

We created the `biology` factor previously, which is a interacting factor between disease status and tissue regions, we therefore will use this directly. 
...
It is recommended to filter out genes with low coverage in the dataset to allow a more accurate mean-variance relationship and reduce the number of statistical tests. Here we use the `filterByExpr` function from the `edgeR` package to filter genes based on the model matrix, while keeping as many genes as possible with worthwhile counts.
...
In the `limma-voom` pipeline, linear modelling is carried out on the log-CPM values by using the `voom`, `lmFit`, `contrasts.fit` and `eBayes` function. If we want fold-change threshold we can also include the `treat` function to perform fold-change requirement to identify DE genes.

**Notes: if we observe variation of patients (individuals) in the PCA plots previously after normalised/batch corrected. It is recommended to accoun for it by using the `duplicateCorrelation`, followed by passing the resulting correlation to the `lmFit` function.**
...
If you're interested in whether some specific genes are DE in the contrast, you can extract them from the DE tables. However, if there isn't a specific list of genes, it is common to perform a gene sets enrichment analysis (GSEA) to find out the enriched gene sets, which might indicate relevant or interest biological patterns.
...
Here are some pre-processing steps of the gene sets, basically we filter out gene sets with less than 5 genes, and creating indices vectors list for easy formatting after running fry.
...
The output is a `data.frame` object. So we can either output the whole table, or just have a look at the top 20 gene sets in a bar plot.

We can see some glomerular-related gene sets such as glomerular_basement_membrane_development and glomerular_epithelial_cell_differentiation are significantly enriched in up-regulated genes in abnormal glomerulus compared to tubule neg. We can also see some diabetic-related pathways enriched.
...
A typical vissE analysis produces three plots: a word-cloud, a network and a gene statistic plot. The word-cloud plot performs a text-mining analysis to automatically annotate gene set clusters (top 9 in this case, ordered by cluster size and the -log10 of the FDR); the network plot visualises gene sets as a network where nodes are gene-sets and edges connect gene-sets that have genes in common; gene statistic plots visualise a gene-specific statistic (a log fold-change in this case) for all genes that belong to gene-sets in the cluster against the number of gene-sets that gene belongs to. Combined, these three plots enable us to identify higher-order biological processes, characterise these processes (word-clouds), assess the relationships between higher-order processes (network plot), and relate the experiment-specific statistics back to the identified processes (gene statistic plot), thereby providing an integrated view of the data.
